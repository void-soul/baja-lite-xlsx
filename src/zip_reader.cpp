...
const char* kRelationshipRelsPath = "xl/_rels/workbook.xml.rels";
...
// Drops <Relationship> elements whose Type is vendor-specific.
size_t stripNonStandardRelationships(const std::string& xml, std::string& out) {
    out.clear();
    out.reserve(xml.size());
    size_t removed = 0;
    size_t pos = 0;
    while (true) {
        const size_t relPos = xmlp::findTagOpen(xml, "Relationship", pos);
        if (relPos == std::string::npos) break;

        const size_t gt = xml.find('>', relPos);
        if (gt == std::string::npos) break;
        const size_t selfClose = xml.find("/>", relPos);
        size_t elemEnd = 0;
        if (selfClose != std::string::npos && selfClose <= gt) {
            elemEnd = selfClose + 2;
        } else {
            const size_t closeTag = xml.find("</Relationship>", gt);
            elemEnd = (closeTag == std::string::npos) ? gt + 1 : closeTag + 15;
        }

        std::string type;
        xmlp::getAttribute(xml, relPos, "Type", type);
        if (isStandardRelationshipType(type)) {
...
bool createSanitizedCopy(const std::string& source, std::string& outTempPath,
    ...
    for (zip_int64_t i = 0; ok && i < numEntries; ++i) {
        ...
        if (!filename.empty() && filename.back() == '/') {
            if (zip_dir_add(out, filename.c_str(), ZIP_FL_ENC_UTF_8) < 0) {
        ...
        if (filename == kRelationshipRelsPath) {
            std::string filtered;
            stripped = stripNonStandardRelationships(payload, filtered);
        ...
        if (!payloadCopy) {
            error = "Out of memory staging entry '" + filename + "'";
            ok = false;
            break;
        }
        if (payloadSize > 0) {
        ...
        if (!src) {
            ...
            error = "Failed to stage entry '" + filename + "'";
            ok = false;
            break;
        }
        if (zip_file_add(out, filename.c_str(), src, ZIP_FL_ENC_UTF_8) < 0) {
            zip_source_free(src);
            error = "Failed to write entry '" + filename + "'";
            ok = false;
            break;
        }
    }
    ...
    if (!ok || stripped == 0) {
        if (stripped == 0 && ok) {
            error = "No non-standard relationship types found";
        }
